...
#         # tree = self._tree
#
#         table_types = ('Event', 'Origin', 'Netmag',
#                        'Stamag', 'Assoc', 'Arrival')
#         Event, Origin, Netmag, Stamag, Assoc, Arrival = _get_entities(
#             query, *table_types)
#
...
#     # @property
#     # def events(self):
#     #     events = self._tree.values()
#     #     return [self.event(event['instance']) for event in events]
#
...
#         return qevent
#
#     # def origins(self, event):
#     #     origins = self._tree[_key(event)]['origins']
#     #     return [self.origin(origin) for origin in origins]
#
...
#         )
#
#         # this should be in the "catalog" function, which receives an QMLTree?
#         tree = self._tree
...
#                     qevent.event_type = FDSN_EVENT_TYPE.get(
#                         origin.etype, "not reported")
#
...
#                             arrival, pick_id=qpick.resource_id)
#                         amplitude.waveform_id = qpick.waveform_id
#
#                     if assoc := assocs[assockey]:
#                         qarrival = self.arrival(
#                             assoc, pick_id=qpick.resource_id)
...
#                         netmag, origin_id=qorigin.resource_id)
...
########
# Make each of these an independent function.
# Split catalog into process_results and catalog.
